...
void clear_map();
void create_level(S_OBJECT* MARIO, int lvl, int& SCORE);
void delete_moving(int i);
S_OBJECT* get_new_brick();
S_OBJECT* get_new_moving();
void horizon_move_map(S_OBJECT* MARIO, float dx);
void horizon_move_object(S_OBJECT* obj, S_OBJECT* MARIO, int& LEVEL, int& MAX_LVL, int& SCORE);
...
void mario_collision(S_OBJECT* MARIO, int& LEVEL, int& MAX_LVL, int& SCORE);
void player_dead(S_OBJECT* MARIO, int& LEVEL, int& SCORE);
...
void show_map();
void vert_move_object(S_OBJECT* obj, S_OBJECT* MARIO,  int& LEVEL, int& MAX_LVL, int& SCORE);
...
int main() {
    ...
    create_level(&MARIO, LEVEL, SCORE);
    ...
    do {
        ...
        if (GetKeyState('A') < 0)
            horizon_move_map(&MARIO, 1.0f);
        if (GetKeyState('D') < 0)
            horizon_move_map(&MARIO, -1.0f);

        if (MARIO.y > MAP_HEIGHT)
            player_dead(&MARIO, LEVEL, SCORE);

        vert_move_object(&MARIO, &MARIO, LEVEL, MAX_LVL, SCORE);
        mario_collision(&MARIO, LEVEL, MAX_LVL, SCORE);
...
void horizon_move_object(S_OBJECT* obj, S_OBJECT* MARIO, int& LEVEL, int& MAX_LVL, int& SCORE) {
    ...
    if (obj->c_type == 'o') {
        S_OBJECT tmp = *obj;
        vert_move_object(&tmp, MARIO, LEVEL, MAX_LVL, SCORE);
        if (tmp.is_fly == true) {
            obj->x -= obj->horiz_speed;
            obj->horiz_speed = -obj->horiz_speed;
        }
